...
const MenuTree = ({ data, isCategoryLoad }) => {
  ...
  const mapNode = (node, parentSlug = '') => {
    ...
    const currentSlug = node.slug ? node.slug : ''; // Default to empty string if slug is missing
    const fullLink = isNaN(parentSlug) ? `${parentSlug}/${currentSlug}` : currentSlug; // Build link dynamically

    console.log('Full Link:', parentSlug, fullLink); // Debugging log to check full link
  ...
  useEffect(() => {
    ...
    node.each(function (d) {
      ...
      // Decide whether this node should be wrapped in <a>
